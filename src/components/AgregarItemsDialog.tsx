                  ...
                  <div key={item.producto.uuid} className="space-y-1">
                    <div className="flex items-center justify-between text-sm">
                      ...
                      <div className="flex items-center gap-1.5 shrink-0">
                        <button onClick={() => updateQty(item.producto.uuid, -1)} className="p-1 rounded bg-secondary text-muted-foreground hover:text-foreground"><Minus size={12} /></button>
                    ...
                    <input type="text" placeholder="Notas…" value={item.notas}
                      ...
                      className="w-full text-xs bg-secondary/50 border border-border rounded px-2 py-0.5 text-muted-foreground focus:outline-none focus:border-primary/50" />
                  </div>
                ))}
                {extraItems.map((e) => (
